...
from mcat.providers.common.arguments import (
    init_providers_parser,
    validate_provider_arguments,
)


class mcatArgumentParser:
    ...
    def __init__(self):
        ...
        self.parser = argparse.ArgumentParser(
            ...
            usage="mcat [-h] [--version] {aws,azure,gcp,kubernetes,dashboard} ...",
            epilog="""
...
To see the different available options on a specific component, run:
    mcat {provider|dashboard} -h|--help

Detailed documentation at https://docs.mcat.com
""",
        )
        # Default
    ...
    def __init_outputs_parser__(self):
        ...
        common_outputs_parser.add_argument(
            ...
            "-F",
            nargs="?",
            help="Custom output report name without the file extension, if not specified will use default output/mcat-output-ACCOUNT_NUM-OUTPUT_DATE.format",
        )
        common_outputs_parser.add_argument(
            "--output-directory",
            "-o",
            nargs="?",
            help="Custom output directory, by default the folder where mcat is stored",
        ...
        common_outputs_parser.add_argument(
            "--no-banner", "-b", action="store_true", help="Hide mcat banner"
        )
        common_outputs_parser.add_argument(
            "--unix-timestamp",
            action="store_true",
    ...
    def __init_logging_parser__(self):
        ...
        common_logging_parser.add_argument(
            "--only-logs",
            action="store_true",
            help="Print only mcat logs by the stdout. This option sets --no-banner.",
        )
    ...
    def __init_checks_parser__(self):
        ...
        common_checks_parser.add_argument(
            ...
            "-x",
            nargs="?",
            help="Specify external directory with custom checks (each check must have a folder with the required files, see more in https://docs.mcat.cloud/en/latest/tutorials/misc/#custom-checks).",
        )
    ...
    def __init_mutelist_parser__(self):
        ...
        mutelist_subparser.add_argument(
            ...
            "-w",
            nargs="?",
            help="Path for mutelist YAML file. See example mcat/config/<provider>_mutelist.yaml for reference and format. For AWS provider, it also accepts AWS DynamoDB Table, Lambda ARNs or S3 URIs, see more in https://docs.mcat.cloud/en/latest/tutorials/mutelist/",
        )
    ...
    def __init_custom_checks_metadata_parser__(self):
        ...
        custom_checks_metadata_subparser.add_argument(
            ...
            nargs="?",
            default=None,
            help="Path for the custom checks metadata YAML file. See example mcat/config/custom_checks_metadata_example.yaml for reference and format. See more in https://docs.mcat.cloud/en/latest/tutorials/custom-checks-metadata/",
        )
    ...
    def __init_third_party_integrations_parser__(self):
        ...
        third_party_subparser.add_argument(
            "--slack",
            action="store_true",
            help="Send a summary of the execution with a Slack APP in your channel. Environment variables SLACK_API_TOKEN and SLACK_CHANNEL_NAME are required (see more in https://docs.mcat.cloud/en/latest/tutorials/integrations/#slack).",
        )
