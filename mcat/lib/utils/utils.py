...
from mcat.config.config import enconding_format_utf_8
...
def open_file(input_file: str, mode: str = "r") -> TextIOWrapper:
    ...
    try:
        ...
        if os_error.strerror == "Too many open files":
            logger.critical(
                "Ooops! You reached your user session maximum open files. To solve this issue, increase the shell session limit by running this command `ulimit -n 4096`. For more info visit https://docs.mcat.cloud/en/latest/troubleshooting/"
            )
        else:
            logger.critical(
