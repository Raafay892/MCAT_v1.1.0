...
from mcat.config.config import mcat_version, timestamp_utc
from mcat.lib.logger import logger
from mcat.lib.outputs.finding import Finding
from mcat.lib.outputs.output import Output
from mcat.lib.utils.utils import hash_sha512
            ...
            for finding in findings:
                ...
                self._data.append(
                    AWSSecurityFindingFormat(
                        ...
                        Id=f"mcat-{finding.check_id}-{finding.account_uid}-{finding.region}-{hash_sha512(finding.resource_uid)}",
                        ProductArn=f"arn:{finding.partition}:securityhub:{finding.region}::product/mcat/mcat",
                        ProductFields=ProductFields(
                            mcatResourceName=finding.resource_uid,
                        ),
                        GeneratorId="mcat-" + finding.check_id,
...
class ProductFields(BaseModel):
    ...
    Attributes:
        - ProviderName (str): The name of the provider, default value is "mcat".
        - ProviderVersion (str): The version of the provider, fetched from the mcat_version in config.py.
        - mcatResourceName (str): The name of the mcat resource.
    """

    ProviderName: str = "mcat"
    ProviderVersion: str = mcat_version
    mcatResourceName: str
