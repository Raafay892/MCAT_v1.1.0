...
mcat_version = "1.0"
html_logo_url = "https://github.com/mcat-cloud/mcat/"
square_logo_img = "https://mcat.com/wp-content/uploads/logo-html.png"
...
def check_current_version():
    try:
        mcat_version_string = f"mcat {mcat_version}"
        release_response = requests.get(
            "https://api.github.com/repos/mcat-cloud/mcat/tags", timeout=1
        )
        latest_version = release_response.json()[0]["name"]
        if latest_version != mcat_version:
            return f"{mcat_version_string} (latest is {latest_version}, upgrade for the latest features)"
        else:
            return (
                f"{mcat_version_string} (You are running the latest version, yay!)"
            )
    except requests.RequestException:
        return f"{mcat_version_string}"
    except Exception:
        return f"{mcat_version_string}"
...
def load_and_validate_config_file(provider: str, config_file_path: str) -> dict:
    """
    Reads the mcat config file in YAML format from the default location or the file passed with the --config-file flag.
...
def load_and_validate_fixer_config_file(
    ...
    """
    Reads the mcat fixer config file in YAML format from the default location or the file passed with the --fixer-config flag.
