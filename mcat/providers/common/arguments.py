...
from mcat.providers.common.provider import Provider, providers_path
...
def init_providers_parser(self):
    """init_providers_parser calls the provider init_parser function to load all the arguments and flags. Receives a mcatArgumentParser object"""
...
def validate_provider_arguments(arguments: Namespace) -> tuple[bool, str]:
    ...
    try:
        # Provider function must be located at mcat.providers.<provider>.lib.arguments.arguments.validate_arguments
        return getattr(
            import_module(
