...
from mcat.providers.aws.aws_provider import get_aws_available_regions
from mcat.providers.aws.config import ROLE_SESSION_NAME
from mcat.providers.aws.lib.arn.arn import arn_type
...
def init_parser(self):
    ...
    aws_auth_subparser.add_argument(
        ...
        nargs="?",
        default=None,
        help="AWS profile to launch mcat with",
    )
    aws_auth_subparser.add_argument(
        "--role",
        "-R",
    ...
    aws_auth_subparser.add_argument(
        ...
        help="An identifier for the assumed role session. Defaults to mcatAssessmentSession",
    ...
    aws_regions_subparser.add_argument(
        ...
        "-f",
        nargs="+",
        help="AWS region names to run mcat against",
    ...
    aws_security_hub_subparser.add_argument(
        "--skip-sh-update",
        action="store_true",
        help="Skip updating previous findings of mcat in Security Hub",
    )
    aws_security_hub_subparser.add_argument(
        "--send-sh-only-fails",
        action="store_true",
        help="Send only mcat failed findings to SecurityHub",
    )
    # AWS Quick Inventory
    ...
    aws_quick_inventory_subparser.add_argument(
        ...
        "-i",
        action="store_true",
        help="Run mcat Quick Inventory. The inventory will be stored in an output csv by default",
    )
    # AWS Outputs
    ...
    aws_based_scans_parser.add_argument(
        ...
        nargs="+",
        default=None,
        help="Scan only resources with specific AWS Tags (Key=Value), e.g., Environment=dev Project=mcat",
    )
    aws_based_scans_parser.add_argument(
        "--resource-arn",
        "--resource-arns",
    ...
    # mcat Fixer
    mcat_fixer_subparser = aws_parser.add_argument_group("mcat Fixer")
    mcat_fixer_subparser.add_argument(
        "--fixer",
        action="store_true",
        help="Fix the failed findings that can be fixed by mcat",
    )
