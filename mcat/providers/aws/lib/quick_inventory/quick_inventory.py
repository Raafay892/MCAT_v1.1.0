...
from mcat.providers.aws.lib.arn.models import get_arn_resource_type
...
def create_output(resources: list, provider: AwsProvider, args):
    try:
        ...
        if not hasattr(args, "output_filename") or args.output_filename is None:
            output_file = (
                f"mcat-inventory-{provider.identity.account}-{output_file_timestamp}"
            )
        else:
        ...
        csv_file.close()
        print(
            f"\n{Fore.YELLOW}WARNING: Only resources that have or have had tags will appear (except for IAM and S3).\nSee more in https://docs.mcat.cloud/en/latest/tutorials/quick-inventory/#objections{Style.RESET_ALL}"
        )
        ...
        if args.output_bucket or args.output_bucket_no_assume:
            ...
            if "mcat/" in bucket_remote_dir:  # Check if it is not a custom directory
                bucket_remote_dir = bucket_remote_dir.partition("mcat/")[-1]
            # CSV
