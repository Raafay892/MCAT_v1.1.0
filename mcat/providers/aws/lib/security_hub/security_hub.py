...
from mcat.config.config import timestamp_utc
from mcat.lib.logger import logger
from mcat.lib.outputs.asff.asff import AWSSecurityFindingFormat

SECURITY_HUB_INTEGRATION_NAME = "mcat/mcat"
...
class SecurityHub:
    ...
    def verify_enabled_per_region(
        ...
        """
        Filters the given list of regions where AWS Security Hub is enabled and returns a dictionary containing the region and their boto3 client if the region and the mcat integration is enabled.
        ...
        for region in aws_security_hub_available_regions:
            try:
                ...
                # Check if mcat integration is enabled in Security Hub
                security_hub_mcat_integration_arn = f"arn:{self._aws_partition}:securityhub:{region}:{self._aws_account_id}:product-subscription/{SECURITY_HUB_INTEGRATION_NAME}"
                if security_hub_mcat_integration_arn not in str(
                    security_hub_client.list_enabled_products_for_import()
                ):
                    logger.warning(
                        f"Security Hub is enabled in {region} but mcat integration does not accept findings. More info: https://docs.mcat.cloud/en/latest/tutorials/aws/securityhub/"
                    )
                else:
    ...
    def archive_previous_findings(self) -> int:
        ...
        for region in self._findings_per_region.keys():
            try:
                ...
                findings_filter = {
                    "ProductName": [{"Value": "mcat", "Comparison": "EQUALS"}],
