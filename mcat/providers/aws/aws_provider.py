...
from mcat.lib.check.check import list_modules, recover_checks_from_service
from mcat.lib.logger import logger
from mcat.lib.utils.utils import open_file, parse_json_file, print_boxes
...
from mcat.providers.aws.lib.arn.arn import parse_iam_credentials_arn
from mcat.providers.aws.lib.arn.models import ARN
from mcat.providers.aws.lib.mutelist.mutelist import AWSMutelist
from mcat.providers.aws.lib.organizations.organizations import (
...
class AwsProvider(Provider):
            ...
            for resource in self._audit_resources:
                ...
                if service != "wafv2" and service != "waf":
                    ...
                    if service == "lambda":
                        ...
                        service = "cloudwatch"
                    # Check if mcat has checks in service
                    try:
    ...
    def set_session_config(self, aws_retries_max_attempts: int) -> Config:
        """
        set_session_config returns a botocore Config object with the mcat user agent and the default retrier configuration if nothing is passed as argument
        """
