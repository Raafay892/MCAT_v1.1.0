...
from mcat.config.config import (
    csv_file_suffix,
...
from mcat.lib.banner import print_banner
...
from mcat.lib.check.checks_loader import load_checks_to_execute
from mcat.lib.check.compliance import update_checks_metadata_with_compliance
from mcat.lib.check.custom_checks_metadata import (
    parse_custom_checks_metadata_file,
    update_checks_metadata,
)
from mcat.lib.cli.parser import mcatArgumentParser
from mcat.lib.logger import logger, set_logging_config
from mcat.lib.outputs.asff.asff import ASFF
from mcat.lib.outputs.compliance.aws_well_architected.aws_well_architected import (
    AWSWellArchitected,
)
from mcat.lib.outputs.compliance.cis.cis_aws import AWSCIS
from mcat.lib.outputs.compliance.cis.cis_azure import AzureCIS
from mcat.lib.outputs.compliance.cis.cis_gcp import GCPCIS
from mcat.lib.outputs.compliance.cis.cis_kubernetes import KubernetesCIS
from mcat.lib.outputs.compliance.compliance import display_compliance_table
from mcat.lib.outputs.compliance.ens.ens_aws import AWSENS
from mcat.lib.outputs.compliance.generic.generic import GenericCompliance
from mcat.lib.outputs.compliance.iso27001.iso27001_aws import AWSISO27001
from mcat.lib.outputs.compliance.mitre_attack.mitre_attack_aws import AWSMitreAttack
from mcat.lib.outputs.compliance.mitre_attack.mitre_attack_azure import (
    AzureMitreAttack,
)
from mcat.lib.outputs.compliance.mitre_attack.mitre_attack_gcp import GCPMitreAttack
from mcat.lib.outputs.csv.csv import CSV
from mcat.lib.outputs.finding import Finding
from mcat.lib.outputs.html.html import HTML
from mcat.lib.outputs.ocsf.ocsf import OCSF
from mcat.lib.outputs.outputs import extract_findings_statistics
from mcat.lib.outputs.slack.slack import Slack
from mcat.lib.outputs.summary_table import display_summary_table
from mcat.providers.aws.lib.s3.s3 import S3
from mcat.providers.aws.lib.security_hub.security_hub import SecurityHub
from mcat.providers.common.provider import Provider
from mcat.providers.common.quick_inventory import run_provider_quick_inventory


def mcat():
    # Parse Arguments
    # Refactor(CLI)
    parser = mcatArgumentParser()
    args = parser.parse()
    ...
    # it is time to check what mcat's checks are going to be executed
    ...
    if global_provider.output_options.fixer:
        print(f"{Style.BRIGHT}\nRunning mcat Fixer, please wait...{Style.RESET_ALL}")
    ...
    if args.slack:
        ...
        if "SLACK_API_TOKEN" in environ and (
            ...
            mcat_args = " ".join(sys.argv[1:])
            slack = Slack(token, channel, global_provider)
            _ = slack.send(stats, mcat_args)
        else:
            # Refactor(CLI)
            logger.critical(
                "Slack integration needs SLACK_API_TOKEN and SLACK_CHANNEL_NAME environment variables (see more in https://docs.mcat.cloud/en/latest/tutorials/integrations/#slack)."
            )
            sys.exit(1)
